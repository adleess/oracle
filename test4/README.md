...
## 实验步骤
### 步骤一 用户创建和空间分配
创建新的表空间和用户wpk，并分配给wpk表空间的查询权限以及创建视图的权限
#### 1） 创建表空间
...
```sql
CREATE USER wpk IDENTIFIED BY 123
...
* 给用户wpk分配表空间的使用配额
```sql
ALTER USER wpk QUOTA UNLIMITED ON USERS;
ALTER USER wpk QUOTA UNLIMITED ON USERS02;
```
* 分配角色CONNEC、RESOURCE，以便用户可以连接数据库，创建资源。
```sql
ALTER USER wpk ACCOUNT UNLOCK;
GRANT "CONNECT" TO wpk WITH ADMIN OPTION;
GRANT "RESOURCE" TO wpk WITH ADMIN OPTION;
```
*分配系统权限"CREATE VIEW"，以便创建视图

```sql
ALTER USER wpk DEFAULT ROLE "CONNECT","RESOURCE";
GRANT CREATE VIEW TO wpk WITH ADMIN OPTION;
```
![](p1.png)
### 步骤二 删除已经存在的表和序列
删除表和序列
...
![](p9.png)
##### ② 创建本地分区索引
在wpk的ORDERS表中创建本地分区索引ORDERS_INDEX_DATE
```sql
...
在wpk的ORDERS表中创建本地分区索引ORDERS_INDEX_CUSTOMER_NAME
```sql
...
##### ③ 设置唯一索引ODERS_PK
在wpk用户的ORDERS表中创建全局分区索引ORDERS_PK
```sql
